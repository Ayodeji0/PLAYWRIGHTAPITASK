...
// POST BY ID AND RETRIVAL AND VALIDATON OF DATA
test("Get Only the Created Post by ID", async ({ request }) => {
    ...
    // GET REQUEST to retrieve the created post by its ID
    ...
    // Validate the details of the created post
    expect(retrievedPost.id).toBe(newPostId);
    expect(retrievedPost.title).toBe(newPostData.title);
    expect(retrievedPost.body).toBe(newPostData.body);
    expect(retrievedPost.userId).toBe(newPostData.userId);
  });
...
test.only("Check the Number of Posts to Ensure Integrity", async ({ request }) => {
